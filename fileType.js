...
var MIME_TABLE = {
    ...
    "d0cf11e0a1b11ae10000": "vsd", //MS Excel 注意：word、msi 和 excel的文件头
...
var fd = fs.openSync( "/Users/heping/Desktop/最终版简历.docx", "r" );
var magicBuffer = Buffer.alloc( 10, 0, "binary" );
fs.read( fd, magicBuffer, 0, magicBuffer.length, null, ( error, bytesRead ) => {
    if ( !error ) {
        var decoder = new StringDecoder( "hex" );
        var magic = decoder.end( magicBuffer );
        var mime = MIME_TABLE[ magic ];
        mime = typeof ( mime ) == "string" ? mime : "oct/stream";
        console.log( 'File mime type is ' + mime );
    } else {
        console.log( error );
    }
} );